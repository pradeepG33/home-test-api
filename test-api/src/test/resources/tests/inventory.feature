...
And match each response.data == { id: '#notnull', name: '#string', price: '#string', image: '#string' }
...
# price for ID 3 is $10
And match response.price == '$10'
And match response.image == 'roll.png'

Scenario: Add item with new ID
Given path 'inventory', 'add'
And request { id: 16, name: 'Evergreen3', image: 'hawaiian.png', price: '$20' }
When method POST
Then status 200
...
Given path 'inventory', 'add'
And request { name: 'Missing ID', image: 'hawaiian.png', price: '$10' }
When method POST
Then status 400
# error returns plain text
...
When method GET
Then status 200
# items now wrapped under data
And match response.data contains { id: 16, name: 'Evergreen3', image: 'hawaiian.png', price: '$20' }
